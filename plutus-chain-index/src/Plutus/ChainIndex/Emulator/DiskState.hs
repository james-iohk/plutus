...
import           Ledger                  (Address (..), Script, ScriptHash, TxOut (..), TxOutRef, Value)
...
txAssetClassMap =
  ...
  where
    ...
    assetClassesOfTxOut TxOut { txOutValue } = assetClassesOfValue txOutValue

    assetClassesOfValue :: Value -> [AssetClass]
    assetClassesOfValue v =
