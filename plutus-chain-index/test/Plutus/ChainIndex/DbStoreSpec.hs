...
module Plutus.ChainIndex.DbStoreSpec (tests) where
...
import           Plutus.ChainIndex                        (Page, PageQuery)
import           Plutus.ChainIndex.ChainIndexError        (ChainIndexError (..))
import           Plutus.ChainIndex.DbStore                (DbStoreEffect, handleDbStore, selectPage)
...
runDbStoreEffectInGenTestDb
    :: Set Int
    -> Eff '[DbStoreEffect, Error ChainIndexError, IO] a
    -> (a -> PropertyT IO ())
    -> PropertyT IO ()
runDbStoreEffectInGenTestDb items effect runTest = do
  result <- liftIO $ Sqlite.withConnection ":memory:" $ \conn -> do
    Sqlite.runBeamSqlite conn $ do
      ...
      liftIO $ runDbStoreEffect conn effect
...
runDbStoreEffect conn effect = do
  effect
